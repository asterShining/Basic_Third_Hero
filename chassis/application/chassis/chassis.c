...
static uint8_t last_follow_brake_request = 0u; // What: 记录跟随刹停请求上一拍电平；Why: V 掉头结束时需要识别刹停请求释放边沿，才能把跟随滤波状态重新贴回当前误差而不是沿用刹停期的零值。
...
static void RefereeUIUpdateData(void)
{
...
#ifdef USE_SUPER_CAP
    uint8_t cap_command_on = 0u;
    uint8_t cap_output_enabled = 0u;
#endif

    // What: 先取一份当前裁判系统对底盘输出的许可状态；Why: 小陀螺和 cap 的显示都应该反映“这拍是否真的允许输出”，不能继续只看抽象模式或慢一拍的回包位。
    if (referee_data != NULL) {
        chassis_output_allowed = (uint8_t)(referee_data->GameRobotState.power_management_chassis_output != 0u);
    }
    ...
    if (chasiss_can_comm != NULL && CANCommIsOnline(chasiss_can_comm) != 0u) {
        ...
        // What: 只有当前仍在自旋执行态且裁判没有切掉底盘输出时才点亮小陀螺；Why: 用户关掉小陀螺或被裁判断电后，on_2 必须立刻转红，不能继续借旧模式或残余状态误亮。
        ...
        // What: 双板断链时主动熄灭小陀螺指示；Why: on_2 必须反映当前可确认的真实模式，不能继续沿用上一帧旧状态误导选手端。
    ...
    ui_data.friction_on = 0u;
    // What: 单板模式下同样按本拍真实自旋执行态点亮 on_2；Why: 即使没有双板链路，也不能让 UI 继续把“模式枚举”和“实际仍在自旋输出”混成一回事。
    chassis_rotate_active = (uint8_t)(chassis_cmd_recv.chassis_mode == CHASSIS_ROTATE && chassis_output_allowed != 0u);
    ui_data.robot_spin_on = chassis_rotate_active;
#endif

#ifdef USE_SUPER_CAP
    // What: 先锁存上板这拍是否明确请求超电介入；Why: cap 灯首先要表达“上层有没有让超电参与”，而不是只看超电板上一拍回了什么状态位。
    cap_command_on = (uint8_t)(chassis_cmd_recv.cap_mode == SUPER_CAP_ON);
    if (SuperCapIsOnline(cap) != 0u) {
        // What: 超电在线时优先显示其回传的真实底盘功率；Why: 功率值本身就该尽量贴近真实电源链路表现，而不是退回到底盘侧估算。
        ui_data.chassis_power_w = SuperCapGetChassisPower(cap);
        // What: cap 只在“本拍命令仍要求开启 + 本地最终确实保持 DCDC 使能 + 当前没有被错误保护打断”时点亮；Why: 之前直接看超电回包 bit7 容易慢一拍，导致已经关了 UI 还残留绿色。
        cap_output_enabled = (uint8_t)(cap->tx_msg.enableDCDC != 0u &&
                                       chassis_output_allowed != 0u &&
                                       SuperCapGetErrorCode(cap) == 0u);
        ui_data.cap_on = (uint8_t)(cap_command_on != 0u && cap_output_enabled != 0u);
        return;
    }
#endif
...
void ChassisTask()
{
    ...
    follow_brake_request_rise = (uint8_t)(chassis_cmd_recv.follow_brake_request != 0u && last_follow_brake_request == 0u); // What: 检测跟随刹停请求上升沿；Why: 进入 V 掉头期时需要立刻清空历史跟随状态，避免底盘再被旧滤波和前馈带着走一段。
    follow_brake_request_fall = (uint8_t)(chassis_cmd_recv.follow_brake_request == 0u && last_follow_brake_request != 0u); // What: 检测跟随刹停请求释放沿；Why: V 掉头结束后要把普通跟随滤波状态重新贴齐当前误差，避免从刹停期的零误差直接切回时再抽动。
    ...
    case CHASSIS_FOLLOW_GIMBAL_YAW: {
        ...
        float raw_angle_err = chassis_cmd_recv.follow_offset_angle; // What: 缓存当前底盘跟随闭环使用的虚拟前方误差；Why: V 键只转云台后新的前方语义只能作用于跟随回正，不能再直接复用真实物理夹角。
        ...
        if (follow_brake_request_rise != 0u) {
            // What: 跟随刹停请求刚进入时立即清空历史滤波、历史输出并重置时间基准；Why: 用户反馈的前段跟转主要来自旧跟随状态残留，进入 V 掉头期必须第一时间把这些残留切掉。
            follow_angle_err_filtered = 0.0f;
            follow_wz_cmd_limited = 0.0f;
            DWT_GetDeltaT(&follow_control_dwt_cnt);
        }
        if (chassis_cmd_recv.follow_brake_request != 0u) {
            // What: 跟随刹停期间持续把滤波误差压成 0；Why: 这一段底盘只允许按自身余旋做阻尼收敛，绝不能再把云台相对角误差重新积回来。
            follow_angle_err_filtered = 0.0f;
            // What: 跟随刹停期间只按车体自身角速度做阻尼制动；Why: 这样既能平滑停住底盘，又能完全隔离云台角速度前馈和位置误差继续把底盘带走。
            raw_follow_wz = -FOLLOW_TRANSITION_BRAKE_KD * chassis_wz;
            LIMIT_MIN_MAX(raw_follow_wz, -FOLLOW_TRANSITION_MAX_WZ, FOLLOW_TRANSITION_MAX_WZ); // What: 复用现有接管阶段的制动限幅；Why: 保持熟悉且已验证过的刹停力度，不额外引入新参数风险。
        } else {
            if (follow_brake_request_fall != 0u) {
                // What: 跟随刹停刚释放时把滤波误差贴回当前虚拟前方误差并重置时间基准；Why: 掉头完成后恢复普通跟随时必须从当前真实状态平滑接回，而不是沿用刹停期间的零误差状态。
