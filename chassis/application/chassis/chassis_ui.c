...
void RefereeUIUpdateData(void)
{
...
#ifdef USE_SUPER_CAP
    // 先锁存上板这拍是否明确请求超电介入，目的是cap 灯首先要表达“上层有没有让超电参与”，而不是只看超电板上一拍回了什么状态位。
    ...
    if (SuperCapIsOnline(cap) != 0u) {
        ...
        // cap 只在“本拍命令仍要求开启 + 本地最终确实保持 DCDC 使能 + 当前没有被错误保护打断”时点亮，目的是之前直接看超电回包 bit7 容易慢一拍，导致已经关了 UI 还残留绿色。
