...
// 发射参数
#define ONE_BULLET_DELTA_ANGLE 80.0f // 发射一发弹丸拨盘输出端固定转动 80 度，目的是在单发限制位变紧后仍保持每次触发只推进一个明确弹位。
#define REDUCTION_RATIO_LOADER (268.0f / 17.0f) // 拨弹盘减速箱实测减速比为 268:17，使用分数表达式避免手写小数截断后把输出端 80 度行程换算出系统误差。
